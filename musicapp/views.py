...
from musicapp.forms import MusicUploadForm
...
def index(request):
    ...
    # Display Local Songs
    songs_local = list(Song.objects.filter(language='Local').values('id'))
    sliced_ids = [each['id'] for each in songs_local][:5]
    indexpage_local_songs = Song.objects.filter(id__in=sliced_ids)
...
def upload_music(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('musicapp:base'))  # Redirect after POST
    else:
        form = MusicUploadForm()
    return render(request, 'musicapp/upload_music.html', {'form': form})
